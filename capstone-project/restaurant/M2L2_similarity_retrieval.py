...
def print_hits(ids, docs, metas, dists, title: str, max_chars: int = 100):
    print(f"\n=== {title} ===")
...
def retrive_articles(query: str, k: int, where: dict | None = None):
...
ids, docs, metas, dists = retrive_articles(q, k=5, where=None)
...
ids, docs, metas, dists = retrive_articles(q, k = 5, where=where_filter)
...
# Todo
where_filter = {'cuisine': 'Italian'}
ids, docs, metas, dists = retrieve_images_by_image(query_image_path=query_img, k = 5, where=where_filter)

if len(ids) == 0:
    print("⚠️ No results found with current filter.")
else:
    print_hits(ids, docs, metas, dists, title="Todo 1 — an optional metadata filter for recipe images")

print("✅ All Todo Completed")
